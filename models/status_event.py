...
class StatusEvent(BaseModel):
    ...
    def event_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['response.status']):
            raise ValueError("must be one of enum values ('response.status')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "event": obj.get("event"),
            "data": StatusEventData.from_dict(obj["data"]) if obj.get("data") is not None else None
        })
        return _obj
